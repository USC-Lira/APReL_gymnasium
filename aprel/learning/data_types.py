...
class Query:
    ...
    def visualize(self, delay: float = 0.0):
...
class Demonstration(QueryWithResponse):
    ...
    def __init__(
        self, trajectory: Trajectory, query: DemonstrationQuery = None
    ):
        ...
        if query is None:
            ...
            assert np.all(
                np.isclose(query.initial_state, initial_state)
            ), "Mismatch between the query and the response for the demonstration."
...
class PreferenceQuery(Query):
    ...
    def __init__(self, slate: Union[TrajectorySet, List[Trajectory]]):
        ...
        assert isinstance(slate, TrajectorySet) or isinstance(
            slate, list
        ), "Query constructor requires a TrajectorySet object for the slate."
        self.slate = slate
        assert (
            self.K >= 2
        ), "Preference queries have to include at least 2 trajectories."
    ...
    def slate(self, new_slate: Union[TrajectorySet, List[Trajectory]]):
        ...
        self._slate = (
            new_slate
            if isinstance(new_slate, TrajectorySet)
            else TrajectorySet(new_slate)
        )
        self.K = self._slate.size
        self.response_set = np.arange(self.K)

    def visualize(self, delay: float = 0.0) -> int:
        ...
        while selection is None:
            ...
            if (
                not isinteger(selection)
                or int(selection) not in self.response_set
            ):
                selection = None
...
class Preference(QueryWithResponse):
    ...
    def __init__(self, query: PreferenceQuery, response: int):
        ...
        assert response in self.query.response_set, (
            "Response "
            + str(response)
            + " is out of bounds for a slate size of "
            + str(self.query.K)
            + "."
        )
...
class WeakComparisonQuery(Query):
    ...
    def __init__(self, slate: Union[TrajectorySet, List[Trajectory]]):
        ...
        self.slate = slate
        assert self.K == 2, (
            "Weak comparison queries can only be pairwise comparisons, but "
            + str(self.K)
            + " trajectories were given."
        )
    ...
    def visualize(self, delay: float = 0.0) -> int:
        ...
        while selection is None:
            selection = input(
                'Which trajectory is the best? Enter a number (-1 for "About Equal"): '
            )
            if (
                not isinteger(selection)
                or int(selection) not in self.response_set
...
class FullRankingQuery(Query):
    ...
    def __init__(self, slate: Union[TrajectorySet, List[Trajectory]]):
        ...
        assert (
            self.K >= 2
        ), "Ranking queries have to include at least 2 trajectories."
    ...
    def slate(self, new_slate: Union[TrajectorySet, List[Trajectory]]):
        ...
        self.K = self._slate.size
        self.response_set = np.array(
            [list(tup) for tup in itertools.permutations(np.arange(self.K))]
        )

    def visualize(self, delay: float = 0.0) -> List[int]:
        """Visualizes the query and interactively asks for a response.
        ...
        while i < self.K:
            ...
            while selection is None:
                selection = input(
                    "Which trajectory is your #"
                    + str(i)
                    + " favorite? Enter a number [0-"
                    + str(self.K - 1)
                    + "]: "
                )
                if (
                    not isinteger(selection)
                    or int(selection) < 0
                    or int(selection) >= self.K
                ):
                    selection = None
                elif int(selection) in response:
                    print(
                        "You have already chosen trajectory "
                        + selection
                        + " before!"
                    )
                    selection = None
...
def isinteger(input: str) -> bool:
    ...
    assert isinstance(
        input, str
    ), "Invalid input to the isinteger method. The input must be a string."
    try:
        a = int(input)
        return True
    except:
