...
def default_query_distance(queries: List[Query], **kwargs) -> np.array:
    ...
    compatible_types = [
        isinstance(query, PreferenceQuery)
        or isinstance(query, WeakComparisonQuery)
        or isinstance(query, FullRankingQuery)
        for query in queries
    ]
    assert np.all(
        compatible_types
    ), "Default query distance, which you are using for batch selection, does not support the given query types. Consider using a custom distance function. See utils/batch_utils.py."
    assert np.all(
        [query.K == 2 for query in queries]
    ), "Default query distance, which you are using for batch selection, does not support large slates, use K = 2. Or consider using a custom distance function. See utils/batch_utils.py."
