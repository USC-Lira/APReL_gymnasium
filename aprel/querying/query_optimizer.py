...
class QueryOptimizerDiscreteTrajectorySet(QueryOptimizer):
    ...
    def argplanner(self, user: User) -> int:
        ...
        raise NotImplementedError(
            "The planner has not been implemented for the given user model."
        )
    ...
    def optimize(
        ...
        **kwargs,
        ...
        assert optimization_method in [
            ...
            "dpp",
        ], "Unknown optimization_method " + str(optimization_method)
        ...
        elif batch_size == 1 and optimization_method in [
            "greedy",
            "medoids",
            "boundary_medoids",
            "successive_elimination",
        ...
        if optimization_method == "exhaustive_search":
            return self.exhaustive_search(
                acquisition_func, belief, initial_query, **kwargs
            )
        elif optimization_method == "greedy":
            return self.greedy_batch(
                acquisition_func, belief, initial_query, batch_size, **kwargs
            )
        elif optimization_method == "medoids":
            return self.medoids_batch(
                acquisition_func, belief, initial_query, batch_size, **kwargs
            )
        elif optimization_method == "boundary_medoids":
            return self.boundary_medoids_batch(
                acquisition_func, belief, initial_query, batch_size, **kwargs
            )
        elif optimization_method == "successive_elimination":
            return self.successive_elimination_batch(
                acquisition_func, belief, initial_query, batch_size, **kwargs
            )
        elif optimization_method == "dpp":
            return self.dpp_batch(
                acquisition_func, belief, initial_query, batch_size, **kwargs
            )

        raise NotImplementedError(
            "unknown optimization method for QueryOptimizerDiscreteTrajectorySet: "
            + optimization_method
            + "."
        )
    ...
    def exhaustive_search(
        ...
        **kwargs,
        ...
        """
        return self.greedy_batch(
            acquisition_func, belief, initial_query, batch_size=1, **kwargs
        )
    ...
    def greedy_batch(
        ...
        **kwargs,
        ...
        """
        if (
            isinstance(initial_query, PreferenceQuery)
            or isinstance(initial_query, WeakComparisonQuery)
            or isinstance(initial_query, FullRankingQuery)
        ):
            if acquisition_func is random:
                best_batch = [initial_query.copy() for _ in range(batch_size)]
                for i in range(batch_size):
                    best_batch[i].slate = self.trajectory_set[
                        np.random.choice(
            ...
            elif acquisition_func is thompson and isinstance(
                belief, SamplingBasedBelief
            ):
                subsets = np.array(
                    [
                        list(tup)
                        for tup in itertools.combinations(
                            np.arange(belief.num_samples), initial_query.K
                        )
                    ]
                )
                ...
                unique_traj_ids, inverse = np.unique(
                    planned_traj_ids, return_inverse=True
                )
                if len(unique_traj_ids) < initial_query.K:
                    remaining_ids = np.setdiff1d(
                        np.arange(self.trajectory_set.size), unique_traj_ids
                    )
                    ...
                    for i in range(batch_size):
                        ids = np.append(
                            unique_traj_ids,
                            np.random.choice(
                                remaining_ids, size=missing_count, replace=False
                            ),
                        )
                    ...
                    unique_probs = np.array(
                        [
                            np.exp(belief_logprobs[inverse == i]).sum()
                            for i in range(len(unique_traj_ids))
                        ]
                    )
                ...
                acquisition_func is mutual_information
                or acquisition_func is volume_removal
            ):
                subsets = np.array(
                    [
                        list(tup)
                        for tup in itertools.combinations(
                            np.arange(self.trajectory_set.size), initial_query.K
                        )
                    ]
                )
                if len(subsets) < batch_size:
                ...
                assert (
                    initial_query.K == 2
                ), "disagreement acquisition function works only with pairwise comparison queries, i.e., K must be 2."
                subsets = np.array(
                    [
                        list(tup)
                        for tup in itertools.combinations(
                ...
                for ids in subsets:
                    weights = np.array(
                        [sample["weights"] for sample in belief_samples[ids]]
                    )
                    vals.append(
                        acquisition_func(
                            weights, belief_logprobs[ids], **kwargs
                        )
                    )
                vals = np.array(vals)
                inds = np.argpartition(vals, -batch_size)[-batch_size:]
            ...
            elif acquisition_func is regret and isinstance(
                belief, SamplingBasedBelief
            ):
                assert (
                    initial_query.K == 2
                ), "regret acquisition function works only with pairwise comparison queries, i.e., K must be 2."
                subsets = np.array(
                    [
                        list(tup)
                        for tup in itertools.combinations(
        ...
        raise NotImplementedError(
            "greedy batch has not been implemented for the given query and belief types."
        )
    ...
    def medoids_batch(
        ...
        **kwargs,
        ...
        top_queries, vals = self.greedy_batch(
            acquisition_func,
            belief,
            initial_query,
            batch_size=kwargs["reduced_size"],
            **kwargs,
        )
        ...
        if len(medoid_ids) < batch_size:
            ...
            ids_to_add = remaining_ids[
                np.argpartition(remaining_vals, -missing_count)[-missing_count:]
            ]
            medoid_ids = np.concatenate((medoid_ids, ids_to_add))
        return [top_queries[idx] for idx in medoid_ids], vals[medoid_ids]

    def boundary_medoids_batch(
        self,
        acquisition_func: Callable,
        belief: Belief,
        initial_query: Query,
        ...
        **kwargs,
        ...
        assert (
            initial_query.K == 2
        ), "Boundary medoids batch selection method does not support large slates, use K = 2."
        feature_dim = initial_query.slate.features_matrix.shape[1]
        if feature_dim > 7:
            warnings.warn(
                "Feature dimensionality is too high: "
                + str(feature_dim)
                + ". Boundary medoids might be too slow."
            )

        features_diff = [
            query.slate.features_matrix[0] - query.slate.features_matrix[1]
        ...
        if len(simplices) < batch_size:
            ...
            distances = kwargs["distance"](
                [top_queries[i] for i in simplices], **kwargs
            )
            temp_ids = kMedoids(distances, batch_size)
            medoid_ids = simplices[temp_ids]
        return [top_queries[idx] for idx in medoid_ids], vals[medoid_ids]

    def successive_elimination_batch(
        self,
        acquisition_func: Callable,
        belief: Belief,
        initial_query: Query,
