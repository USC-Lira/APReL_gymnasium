...
def regret(
    ...
    **kwargs,
    ...
    """
    assert (
        len(weights) == len(logprobs) == len(planned_trajectories) == 2
    ), "regret acquisition function works only with pairwise comparison queries, i.e., K must be 2."

    term1 = np.prod(np.exp(logprobs))
    term2 = np.dot(weights[0], planned_trajectories[0].features) / np.dot(
        weights[1], planned_trajectories[0].features
    )
    term3 = np.dot(weights[1], planned_trajectories[1].features) / np.dot(
        weights[0], planned_trajectories[1].features
    )
